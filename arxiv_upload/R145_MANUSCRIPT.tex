...
These numbers should not be reduced to a simple success or failure.  In the equal-weight aggregate alone, the fixed GMR prescription has lower RMSE than baryons-only, MOND, and reduced NFW.  That statement does not mean that GMR wins galaxy by galaxy: reduced NFW keeps the smaller median residual and higher per-galaxy win rate, and reduced ISO and Burkert are stronger empirical halo prescriptions overall.  Observation-weighted scoring also changes the ordering: GMR has a full-sample $N_{\rm obs}$-weighted RMSE of 57.334 \kms, whereas MOND gives 52.224 \kms.  The response prescription is therefore a structured constraint, not a dominance claim.  The tabulated RMSE values are point estimates; a complete set of per-model galaxy-bootstrap intervals is not included here.
...
The exponent comparison is mixed.  SPARC equal-weight metrics and the direct LSB component sample favor the fixed $p\simeq0.298$ branch.  LITTLE THINGS proxy tests and the validated vector-recovered subset lean toward $p=0.5$.  The rows below are fixed-branch comparisons, not fresh per-galaxy exponent fits: SQRT fixes $p=0.5$, FREEP fixes $p\simeq0.298$ from the internal calibration track, and the quoted objective is equal-weight per-galaxy RMSE unless stated otherwise.  Because the external evidence combines direct component tables, proxy curves, and vector-recovered components, it cannot provide a single robust posterior interval for a universal $p$.  A full per-data-set fit of $(A_p,p)$ with uncertainty propagation is needed before claiming that the exponent itself has been measured separately in each regime.
...
The vector-recovered subset is intentionally conservative.  Seventeen galaxies pass validation, including fifteen GOLD and two SILVER cases.  The validation gate is independent of the SQRT/FREEP comparison: accepted cases pass the published-curve recovery and closure checks recorded in the validation table, while rejected cases fail one of those gates or lack the necessary machine-readable dark-matter curve.  SQRT wins eleven of the seventeen, with 16.446 \kms\ versus 16.570 \kms\ for FREEP.  A paired sign test gives $p=0.332$; a galaxy-resampling bootstrap for the aggregate SQRT-minus-FREEP RMSE difference gives a 95\% interval of $[-1.76,+1.36]$ \kms\ and $P(\Delta<0)=0.609$.  These values are uncertainty checks, not an equivalence test and not a superiority claim.  Rejected galaxies are not tuned back into the sample.  The result is consistent with the direction of the LITTLE THINGS proxy split, but it does not promote that split to a universal law.

\section{Interpretation}
The fixed-branch comparisons suggest regime sensitivity in the projected response branch, but they do not yet demonstrate it as a fitted physical parameter.  The same one-parameter projection does not behave identically in SPARC, direct LSB mass models, LITTLE THINGS proxy curves, and vector-recovered dwarf-galaxy components.

The channel-aware interpretation is compatible with this behavior, but the data do not establish it on their own.  A projected exponent can absorb changes in baryonic structure, gas dominance, stellar mass-to-light assumptions, response alignment, and metric weighting.  A deeper theory may still contain a more restrictive exponent, but the present evidence does not show that.

\section{Limitations and falsifiability}
The scope is narrower than a dark-sector solution: cluster lensing, cosmology, Solar-System screening, and a closed action are not addressed here.  The LITTLE THINGS component curves used here are vector-recovered from published figures, not official machine-readable $V_{\rm gas}/V_{\rm disk}/V_{\rm bulge}$ tables in the CDS products.  The direct LSB sample is small; the SPARC free-exponent result is internal and metric-dependent; the RMSE tables do not yet attach a full galaxy-bootstrap interval to every comparator; and the summary does not include a full error-weighted $\chi^2$ reanalysis.  The cumulative $Q_{\rm enc}$ feature is also a first-order ordered radial sum, not a grid-invariant continuum functional.

The channel-aware equations are therefore a nonrelativistic phenomenological parent, not a completed relativistic gravity theory.  Any completion must satisfy post-Newtonian and Solar-System constraints, including Cassini-level bounds, and must provide screening or decoupling before the same response law is extrapolated outside galaxy rotation curves \citep{Bertotti2003,Will2014LRR,KhouryWeltman2004}.  The decisive empirical test is to obtain official or independently published direct LITTLE THINGS or THINGS component tables and repeat the no-retuning comparison.  The next version of the analysis should also release the full fixed-exponent scan, add error-weighted residual metrics, and attach bootstrap intervals for the principal RMSE comparisons.

\section{Data availability}
The source files, figure assets, BibTeX database, and LITTLE THINGS vector-validation table are available alongside this article.  External survey products remain with their original archives and should be cited through the SPARC, LITTLE THINGS, and LSB source papers.  The validation table documents the PDF-vector subset; it is not a replacement for official survey component tables.
...
The validation subset used for the LITTLE THINGS vector-recovered check contains 17 accepted galaxies.  For each system, the accompanying table gives the validation class, the number of radial points, and the SQRT/FREEP RMSE values entering the aggregate comparison.  The nine systems rejected by the validation gate, or excluded for lack of a matched dark-matter curve, are kept separate.  This distinction matters: the recovered curves document what can be read from the published figures, but they are not an official LITTLE THINGS component catalogue.

\begingroup
\scriptsize
\bibliographystyle{aasjournal}
\bibliography{R145_REFERENCES}
\endgroup
\end{document}

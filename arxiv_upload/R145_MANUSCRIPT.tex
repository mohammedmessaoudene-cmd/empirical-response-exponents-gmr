...
The scope is narrower than a dark-sector solution: cluster lensing, cosmology, Solar-System screening, and a closed action are outside the present analysis.  The LITTLE THINGS component curves used here are vector-recovered from published figures, not official machine-readable $V_{\rm gas}/V_{\rm disk}/V_{\rm bulge}$ tables in the CDS products.  The direct LSB sample is small; the SPARC free-exponent result is internal and metric-dependent; the RMSE tables lack a full galaxy-bootstrap interval for every comparator; and the summary lacks a full error-weighted $\chi^2$ reanalysis.  The cumulative $Q_{\rm enc}$ feature is also a first-order ordered radial sum, not a grid-invariant continuum functional.
